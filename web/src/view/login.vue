<template>
  <div id="login" class="bg">
    <div style="width:100%">
      <h1 class="title" :hidden="!photo_hidden">实验室指导资料库</h1>
    </div>
    <card :bordered="false" :class="{login:photo_hidden,loginCamera:!photo_hidden} " class="card">
      <p slot="title" class="form_title" v-text="photo_hidden?'登录实验室':'人脸验证'"></p>
      <i-form
        class="form"
        ref="login_form"
        :model="info"
...
export default {
  ...
  methods: {
    ...
    successFunc(stream) {
      ...
      if (that.interval_id == "")
        that.interval_id = setInterval(that.draw_photo, 2500);
      ...
      this.$api.face.check(pack).then(res => {
        if (res.data.code === 1) {
          if (res.data.data > 80) {
            ...
            that.alert_info =
              "人脸相似度为" + parseInt(res.data.data) + "%,过低，请重新校验";
            that.button_enable = true;
          }
        } else if (res.data.code === 0) {
          that.alert_info =
            "百度人脸验证提供的ACCESS TOKEN已过期，请通知管理员进行更新";
          that.button_enable = true;
        ...
        if (
          res.data.code !== 1 ||
          (res.data.code === 1 && res.data.data <= 80)
        ) {
          if (that.photo_num == 3) {
            that.stop_auto_check();
            that.button_enable = true;
